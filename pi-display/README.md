...
- **Compromised sign-in**: if whoever's credentials were used to provision a
  ...
  just _Revoke_ the device — that alone stops it reading anything, no Logto
  change needed). There's no shared secret to rotate application-wide.
...
per-device _Revoke_ (immediate, no Logto change needed), and rotating the
provisioning person's Logto credentials if the stolen refresh token itself
is a concern. Treat physical access to venue displays accordingly.
