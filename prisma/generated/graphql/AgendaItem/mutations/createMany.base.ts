...
export const createManyAgendaItemMutationArgs = builder.args((t) => ({ data: t.field({ type: [Inputs.AgendaItemCreateInput], required: true }) }))
...
export const createManyAgendaItemMutationObject = defineMutationFunction((t) =>
  defineMutationPrismaObject({
    ...
    resolve: async (_query, _root, args, _context, _info) =>
      await db.$transaction(args.data.map((data) => db.agendaItem.create({ data }))),
  }),
);
