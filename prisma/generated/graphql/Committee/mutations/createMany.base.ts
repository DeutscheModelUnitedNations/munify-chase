...
export const createManyCommitteeMutationArgs = builder.args((t) => ({ data: t.field({ type: [Inputs.CommitteeCreateInput], required: true }) }))
...
export const createManyCommitteeMutationObject = defineMutationFunction((t) =>
  defineMutationPrismaObject({
    ...
    resolve: async (_query, _root, args, _context, _info) =>
      await db.$transaction(args.data.map((data) => db.committee.create({ data }))),
  }),
);
