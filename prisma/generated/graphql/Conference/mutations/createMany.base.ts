...
export const createManyConferenceMutationArgs = builder.args((t) => ({ data: t.field({ type: [Inputs.ConferenceCreateInput], required: true }) }))
...
export const createManyConferenceMutationObject = defineMutationFunction((t) =>
  defineMutationPrismaObject({
    ...
    resolve: async (_query, _root, args, _context, _info) =>
      await db.$transaction(args.data.map((data) => db.conference.create({ data }))),
  }),
);
