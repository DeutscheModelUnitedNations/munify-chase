<script lang="ts">
	...
	let { value = $bindable(), options, side }: Props = $props();
...
<Combobox.Root type="single" name="stateOfDebate" bind:value={getValue, setValue}>
	<div class="join">
		<Combobox.Input>
			{#snippet child({ props })}
				<input
...
<!-- <Combobox.Root type="single" name="stateOfDebate" bind:value={getValue, setValue}>
	<div class="relative">
		<i class="fas fa-diagram absolute start-3 top-1/2 size-6 -translate-y-1/2"></i>
		<Combobox.Input
		...
		>
			{#if value}
				<span
					class="text-base-content/50 absolute start-3 top-1/2 -translate-y-1/2 text-sm font-bold"
		...
		<div class="absolute end-3 top-1/2 flex -translate-y-1/2">
			<button
			...
			<Combobox.Trigger class="btn btn-ghost btn-sm btn-square">
				<i class="fas fa-caret-down"></i>
			</Combobox.Trigger>
		</div>
	</div>
	<Combobox.Portal>
		...
		>
			...
			<Combobox.Viewport class="p-1">
				{#each filteredOptions as option, i (i + option.label)}
					<Combobox.Item
						class="btn btn-ghost flex h-10 w-full py-3 pl-5 text-sm outline-hidden select-none"
						value={option.label}
						label={option.label}
						onclick={() => {
							setValue(option.label);
