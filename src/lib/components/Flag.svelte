...
<div
  ...
  'NSA'
    ...
    : representation?.type === 'DELEGATION' &&
      'text-base-content bg-[#bea162]'} {placeholder &&
...
>
  ...
  {:else if representation?.faIcon}
    <i
      class="fa-solid fa-{representation.faIcon?.replace(
        'fa-',
        '',
      )} {iconClassNames()}"
    ></i>
  {:else}
    <span
      class="fi fi-{representation?.type === 'UN'
        ? 'un'
        : representation?.alpha2Code}"
    ></span>
  {/if}
</div>
