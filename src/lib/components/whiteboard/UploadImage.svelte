...
<dialog class="modal" open={showModal}>
  <div class="modal-box bg-base-200">
    ...
    <div class="tabs-box mb-4 tabs justify-stretch bg-base-300">
      <button
      ...
      <button
        class="tab-bordered tab flex-1 {activeTab === 'URL' ? 'tab-active' : ''}"
    ...
    <div class="card bg-base-100 p-4 shadow-sm">
      {#if activeTab === 'UPLOAD'}
        <div class="flex flex-col gap-4">
          ...
          {#if uploadError}
            <div class="text-sm text-error">{uploadError}</div>
          {/if}
