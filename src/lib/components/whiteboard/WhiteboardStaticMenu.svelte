...
<div class="card sticky top-0 z-10 flex flex-row flex-nowrap gap-2 bg-base-300 p-2 shadow-sm">
  {#each buttonGroups as group}
    <div class="join">
      {#each group as button}
        <div class="tooltip tooltip-bottom" data-tip={button.label}>
          <button
            class="btn join-item btn-sm {button.active && button.active() ? 'btn-active' : ''}"
