<script lang="ts">
	...
	let editingUser = $state<(typeof conferenceUsers)[number] | null>(null);
	...
	type ConferenceUserRow = (typeof conferenceUsers)[number];

	// Compute assignment counts per member (how many users share each seat)
	...
	// TanStack Table (framework-agnostic core) with Svelte 5 runes
	const columns: ColumnDef<ConferenceUserRow, unknown>[] = [
		...
		{
			accessorKey: 'conferenceUserType',
			header: 'Role'
		},
		{
			id: 'committee',
			accessorFn: (row) => getCommitteeText(row)
		},
		{
			id: 'assignment',
			accessorFn: (row) => getAssignmentText(row) ?? '',
			enableSorting: false
		},
		{
			id: 'actions',
	...
	// Get default table state (includes columnPinning, columnVisibility, etc.)
	...
	// Reactive table: recreated when data/state changes
	let table = $derived(
		createTable({
							...
							{:else}
								{#each table.getRowModel().rows as row (row.id)}
									...
									<tr>
										...
										<td>
											<span
												class="badge {roleBadgeClass[user.conferenceUserType] ?? 'badge-ghost'}"
											>
												{roleLabel[user.conferenceUserType]?.() ?? user.conferenceUserType}
											</span>
										</td>
										...
										<td>
											{#if getAssignmentRepresentation(user)}
												<div class="flex items-center gap-2">
													<Flag
														representation={getAssignmentRepresentation(user) as any}
														size="xs"
													/>
